...
const getListData = (value: Dayjs, history: ITracker[], schedule: number[]) => {
  let listData = [];
  let scheduleObj = schedule.map((s) => weekDays.find((i) => s == i.id));
  let highlight = scheduleObj.findIndex((i) => value.format("ddd") == i?.title);
...
export default function TaskCalender({
  ...
  const dateCellRender = (value: Dayjs) => {
    const { listData, highlight } = getListData(value, history, schedule);
    console.log("hig", highlight);
    return (
      <Row
        gutter={[8, 8]}
        style={{
          background: highlight > -1 ? "#ff00d447" : "transparent",
          width: "100%",
          height: "100%",
        }}
      >
